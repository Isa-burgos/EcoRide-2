...
<main>
    ...
    <section class="conteneur">
        ...
        <?= csrfField(); ?>
            ...
            <div class="mb-3">
                <div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" id="isPassenger" name="statuts[]" value="Passager"
                            <?= in_array('passager', $statuts) ? 'checked' : '' ?>>
                        <label for="isPassenger" class="form-check-label me-5">Je suis passager</label>
                    </div>
                    <div class="form-check form-check-inline">
    ...
    <section class="conteneur mb-5">
    <div id="vehicle-section" class="p-3 mb-1" hidden>
                    ...
                    <div class="col-12 col-md-12 col-lg-6 mb-3">
                        <div class="card">
                            ...
                            <div class="card-header">
                                <h3 class="card-title">
                                    <?php renderPartial('alert'); ?>
                                    <?= htmlspecialchars($vehicle?->getBrand())?>
                                </h3>
                                <div class="card-subtitle"><?= htmlspecialchars($vehicle?->getModel()) ?> - (<?= htmlspecialchars($vehicle?->getColor()) ?>)</div>
                            </div>
                            <div class="card-body">
                                <p>Immatriculation : <?= htmlspecialchars($vehicle?->getRegistration()) ?></p>
                                <?php if($vehicle->getNbPlace()): ?>
                                    <p><?= $vehicle?->getNbPlace() . " place" . ($vehicle->getNbPlace() > 1 ? 's' : "") ?> disponible<?= $vehicle->getNbPlace() > 1 ? 's' : "" ?></p>
                                <?php endif ?>
                                <?php
                                ...
                                ?>
                                    <?php if (!empty($preferences)): ?>
                                            ...
                                            <?php if (isset($preferences['smoking'])): ?>
                                                <p>Fumeur : <?= $preferences['smoking'] ? 'oui' : 'non' ?></p>
                                            <?php endif; ?>
                                            <?php if (isset($preferences['pets'])): ?>
                                                <p>Animaux acceptés : <?= $preferences['pets'] ? 'oui' : 'non' ?></p>
                                            <?php endif; ?>
                                            <?php if (!empty($preferences['custom'])): ?>
                                                <p><em><?= htmlspecialchars($preferences['custom']) ?></em></p>
                                            <?php endif; ?>
                            ...
                            <div class="card-footer">
                                <a href="/vehicle/<?= $vehicle->getVehicleId() ?>/edit" class="btn-edit mx-2" title="Modifier le véhicule">
                                ...
                                <?= csrfField(); ?>
                                    <button type="submit" class="btn-delete bg-transparent border-0 mx-2" title="Supprimer le véhicule" onclick="return confirm('Supprimer ce véhicule ?')">
...
<form method="POST" action="/vehicle/create" class="container-content p-2">
<?= csrfField(); ?>
...
<section class="conteneur page my-3">
    ...
    <div class="conteneur-content m-auto">
