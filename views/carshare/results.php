<section class="big-hero text-center">
    <div class="big-hero-content">
        <div class="container search-trip-mobile">
            <button class="search-trip-btn" type="button" data-bs-toggle="modal" data-bs-target="#searchModal">
        ...
        <div class="container search-trip-desktop bg-light">
            <button class="search-trip-btn bg-light border-0 w-100" type="button" data-bs-toggle="modal" data-bs-target="#searchModal">
                <div class="search-trip-container d-flex align-items-center my-2">
                    <div class="search-trip-direction align-items-center justify-between-center w-100">
                        <div class="d-flex">
                            <img src="/assets/img/startAdress.svg" alt="" class="me-2">
                            <p class="text-dark me-2"><?= htmlspecialchars($depart_adress) ?></p>
                        </div>
                        <div class="line"></div>
                        <div class="d-flex">
                            <img src="/assets/img/endAdress.svg" alt="" class="me-2">
                            <p class="text-dark me-2"><?= htmlspecialchars($arrival_adress) ?></p>
                        </div>
                        <div class="line"></div>
                        <div class="d-flex">
                            <img src="/assets/img/calendar.svg" alt="" class="me-2">
                            <p class="text-secondary text-start me-2"><?= htmlspecialchars(formatDateFr($depart_date)) ?></p>
                        </div>
                        <div class="line"></div>
                        <p class="text-secondary text-start me-2"> <?= htmlspecialchars($nb_place) ?> passager<?= $nb_place > 1 ? 's' : '' ?></p>
                        ...
                        <a href="/carshare/<?= $carshare->getCarshareId() ?>/details">
                            <div class="trip-card p-0">
                                <div class="trip-details-top">
                                    ...
                                    <div class="trip-details-right pe-2">
                                        <div class="trip-price">
                                            <p class="me-2"><?= htmlspecialchars($carshare->getPricePerson()) ?> <span>crédits</span></p>
                                        </div>
                                        <div class="trip-passenger">
                                            <?php if($carshare->getAvailablePlaces() > 0) : ?>
                                                <p class="me-2"><?= htmlspecialchars($carshare->getAvailablePlaces()) ?><span> passager<?= ($carshare->getAvailablePlaces()) > 1 ? 's' : '' ?></span></p>
                                            <?php else : ?>
                                                <p class="me-2">COMPLET</p>
                                            <?php endif ?>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="trip-details-bottom">
                                    <div class="d-flex">
                                        ...
                                        <div class="profile-photo me-3">
                                            <img class="profile-picture" src="/<?= htmlspecialchars($carshare->getDriver()->getPhoto()) ?>" alt="Photo de profil">
                                        </div>
                                    ...
                                    <div class="trip-icons">
                                        <span class="badge rounded-pill text-white text-bg-primary me-2">
                                            <img src="<?= htmlspecialchars($carshare->getEnergyIcon()) ?>" alt="énergie" width="20">
                                        </span>
                                        <span class="badge rounded-pill text-white text-bg-primary me-2">
                                            <?php $smokingIcon = $carshare->smoking_icon ?? '/assets/icons/no-smoking.svg'; ?>
                                                <img src="<?=htmlspecialchars($carshare->smoking_icon); ?>" alt="">
                                        </span>
                                        <span class="badge rounded-pill text-white text-bg-primary me-2">
                                            <?php $petsIcon = $carshare->pets_icon ?? '/assets/icons/no-pets.svg'; ?>
                                                <img src="<?=htmlspecialchars($carshare->pets_icon); ?>" alt="">
                                        </span>
                                    </div>
                                    <?php endif ?>
                                </div>
...
<!-- Modal barre de recherche -->
<div class="modal fade" id="searchModal" tabindex="-1" aria-labelledby="searchModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content bg-secondary">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="searchModalLabel">Modifier ma recherche</h1>
            ...
            <div class="modal-body">
                <form action="/carshare/search" method="get">
                    ...
                    <div class="champ gx-1">
                        <input class="date-depart form-control input" type="date" name="depart_date" placeholder="Date de départ" value="<?= htmlspecialchars($depart_date) ?>" required min="<?= date('Y-m-d') ?>">
                    </div>
                    <div class="champ gx-1">
                        <input class="passenger form-control input" type="number" name="nb_place" placeholder="Passager" value="<?= htmlspecialchars($nb_place) ?>" min="1">
                    </div>
